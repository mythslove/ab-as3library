...
package com.ab.ui
{
	...
	*/
	...
	public class TapScroller extends Sprite
	{
		...
		public function TapScroller( content:DisplayObject, width:Number, height:Number)
		{
			//_stage = __stage;
			// copy size
			panelWidth = width;
		...
		private function handleMouseMove(e:MouseEvent):void 
		{
			...
			if (useVertical && Math.abs(totalY) > scrollFactor)		{ isDragging = true; }
			
			if (useHorizontal && Math.abs(totalX) > scrollFactor)	{ isDragging = true; }
			
			if (isDragging) 
			{
				if (useVertical) 
				{
					if (totalY < min.y)  { totalY = min.y - Math.sqrt(min.y - totalY); }
					
					if (totalY > max.y)  { totalY = max.y + Math.sqrt(totalY - max.y); }
				...
				if (useHorizontal) 
				{
					if (totalX < min.x)  { totalX = min.x - Math.sqrt(min.x-totalX); }
					
					if (totalX > max.x)  { totalX = max.x + Math.sqrt(totalX - max.x); }
		...
		private function handleMouseUp(e:MouseEvent):void 
		{
			...
			if (useVertical) 	{ inertia.y = diff.y; }
			
			if (useHorizontal)  { inertia.x = diff.x; }
