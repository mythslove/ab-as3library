package com.edigma.xml {
	...
	dynamic public class XMLProperties extends Proxy implements IEventDispatcher {
		...
		private function settingsParser(value : XML) : void {
			...
			//iterate settings
			for each (var xmlItem:XML in settingsIterator) {
				//retrieve setting from current XMLItem				
				var applicationSettingName : String = xmlItem.property.(@id == "settingName").text();
				var applicationSettingValue : String = xmlItem.property.(@id == "settingValue").text();
				//push the new menu into the menu dictionary
